...
系统 MUST 提供 `GET /api/v1/users` 用户列表接口。接口 MUST 支持分页查询，MUST 通过统一响应信封返回分页用户资料列表，且返回用户资料 MUST 不包含内部数据库 `id`、`email`、`password` 或密码哈希。
...
- **Then** 每个用户资料 MUST 包含 `user_id`、`name`、`username`、`active`、`created_at`、`updated_at`
- **Then** 每个用户资料 MUST NOT 包含 `id`、`email`、`password` 或密码哈希
...
用户列表接口 MUST 支持明确白名单过滤参数。系统 MUST 支持 `name`、`username`、`active` 过滤，并忽略未定义的 query 参数。系统 MUST 不再支持 `email` 过滤语义。
...
- **Given** 数据库中存在用户名为 `alice` 的用户
- **When** 调用方请求 `GET /api/v1/users?username=alice`
- **Then** 系统 MUST 按 `username` 过滤用户
- **Then** 返回结果 MUST 只包含用户名匹配 `alice` 的用户

#### Scenario: Ignore email filter
- **Given** 调用方请求 `GET /api/v1/users?email=alice@example.com`
- **When** 系统解析用户列表过滤参数
- **Then** 系统 MUST 将 `email` 视为未定义 query 参数
- **Then** repository MUST NOT 使用 email 字段或 email 谓词构造查询
...
- **Then** 文档 MUST 描述 `page`、`page_size`、`name`、`username`、`active` query 参数
- **Then** 文档 MUST NOT 描述 `email` query 参数
- **Then** 文档 MUST 描述 HTTP 200 成功响应为统一响应信封包装的分页用户资料
- **Then** 文档 MUST 不得把 `id`、`email`、`password` 或密码哈希描述为响应字段
