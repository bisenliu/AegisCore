...
- **AND** tracing instrumentation 失败时 constructor MUST 返回 error 而不是 panic
- **AND** tracing instrumentation 失败时 constructor MUST 关闭已创建 client，并保留 instrumentation failure 和关闭失败信息
- **AND** 启动 PING 失败时 provider MUST 关闭 client，并保留探测失败和关闭失败信息

#### Scenario: 使用 Fx adapter 组合 Redis

- **WHEN** Fx 服务声明一个具名 Redis 资源
- **THEN** `common/runtime/datastore` 中共置的 Fx adapter MUST 调用框架无关的单资源 constructor
- **AND** adapter MUST 只注册该资源的启动 `Ping` 和 `Close` hook
- **AND** adapter MUST NOT 遍历 `RedisConfigs` 或自动创建配置中出现的其他资源
- **AND** constructor 失败时 adapter MUST 返回 error，并保留资源名称和原始 cause
- **AND** 成功连接和关闭日志 MUST 保留 Redis component 与资源名称
