...
- 修正 tracing Fx provider 的初始化时序，使 Redis、Gin、Ent 等 constructor 阶段依赖的 `TracerProvider()` 在构图期间即可用，同时保留 Fx 停止阶段的 provider shutdown。
...
- `runtime-observability`: 扩展运行时日志观测要求，使 Fx 初始化、构图、Invoke、rollback 和 lifecycle 事件必须使用统一结构化 logger 输出，并确保 tracing provider 在依赖它的 constructor 执行前可用。

## Impact

- 影响代码：`common/runtime/logger/fx.go`、`common/runtime/observability/tracing/fx.go`、`user-service/internal/bootstrap/app.go` 以及相关单元测试。
