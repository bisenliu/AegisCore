...
- [ ] 5.8 运行 `make user-service-architecture-lint` 验证 OpenSpec 与架构边界；当前因 `user-service/docs/openapi.go`、`openapi.json` 和 `openapi.yaml` 存在本次预期未提交生成物 diff 被 drift 检查拦截。
