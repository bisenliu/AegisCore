...
系统 MUST 审查并更新用户表索引，使常用查询条件 `username`、`nickname`、`status` 和 `deleted_at IS NULL` 能与新的字段命名和软删除语义一致。
...
#### Scenario: Preserve username uniqueness with soft delete
- **Given** 用户表需要按用户名识别用户
- **When** migration 审查用户名唯一索引
- **Then** 实现 MUST 明确选择全表唯一用户名索引或 `deleted_at IS NULL` partial unique index
