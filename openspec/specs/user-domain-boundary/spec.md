...
系统 SHALL 在 `user-services/internal/user` 中提供用户领域实体，用于表达用户对外身份、基础资料、认证凭据摘要、状态、token version 和公开时间戳。用户状态相关业务判断 MUST 通过用户领域实体或用户状态枚举方法表达，Service 层 MUST NOT 直接依赖 Ent 用户模型字段实现用户状态规则。
...
- **Then** `User` Ent schema MUST NOT 导入 `user-services/internal/user`
- **Then** `status` 字段默认值 MUST 保持为数据库契约值 `100`
- **Then** Service 层状态判断 MUST 继续通过 `user.UserStatus` 或用户领域实体表达
...
- **Then** Store MUST 将 Ent `status` 数值转换为 `user.UserStatus`
