...
系统 SHALL 将 Ent predicate 构建细节封装在 store 内部实现目录中，例如 `user-services/internal/features/user/store/postgres/predicates.go`。App service 层 MUST NOT import `user-services/ent/user`、`user-services/ent/predicate` 或其他 ORM 查询构造细节；所有查询条件拼装 MUST 由 store 内部根据业务语义完成。
