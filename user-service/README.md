...
日志只写 stdout/stderr，tracing 启用后固定使用 OTLP，进程时区使用 `runtime.timezone`。Gin 默认不信任代理；需要在反向代理、Ingress、gateway 或 service mesh 后记录真实客户端地址时，必须在 Nacos 中通过 `server.http.trusted_proxies` 配置真实上游代理 IP/CIDR，并由入口层覆盖或重建 forwarded headers。pprof 默认关闭；临时诊断需修改 Nacos 配置并使用受控端口转发。
