...
func (s *permissionCommandService) EnablePermission(ctx context.Context, cmd SetPermissionActiveCommand) error {
	return s.setPermissionActive(ctx, cmd.PermissionID, true)
}

// DisablePermission 停用权限目录记录，不物理删除记录。
func (s *permissionCommandService) DisablePermission(ctx context.Context, cmd SetPermissionActiveCommand) error {
	return s.setPermissionActive(ctx, cmd.PermissionID, false)
}

func (s *permissionCommandService) setPermissionActive(ctx context.Context, permissionID uuid.UUID, active bool) error {
	err := s.store.SetActive(ctx, permissionID, active)
	if err != nil {
