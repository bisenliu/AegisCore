...
type Lifecycle interface {
	CreateTokenSession(ctx context.Context, userID uuid.UUID, sessionID string, tokenVersion int64, refreshTTL time.Duration) error
	CreatePasswordChangeSession(ctx context.Context, userID uuid.UUID, sessionID string, tokenID string, tokenVersion int64, ttl time.Duration) error
	...
	CurrentTokenVersion(ctx context.Context, userID uuid.UUID) (int64, error)
...
func (m *lifecycle) CreateTokenSession(ctx context.Context, userID uuid.UUID, sessionID string, tokenVersion int64, refreshTTL time.Duration) error {
	if err := m.sessions.CreateSession(ctx, authdomain.AuthSession{UserID: userID, SessionID: sessionID, TokenVersion: tokenVersion, ExpiresAt: time.Now().Add(refreshTTL)}, refreshTTL, m.maxActiveSessionsPerUser); err != nil {
		logger.Error(ctx, "create auth session failed", logger.StackTrace(zap.String("user_id", userID.String()), zap.String("session_id", sessionID), zap.Int64("token_version", tokenVersion), zap.Error(err))...)
		return err
	}
	return nil
}
...
func (m *lifecycle) CreatePasswordChangeSession(ctx context.Context, userID uuid.UUID, sessionID string, tokenID string, tokenVersion int64, ttl time.Duration) error {
	...
	if err := m.passwordChangeSessions.CreatePasswordChangeSession(ctx, session, ttl); err != nil {
		logger.Error(ctx, "create password change session failed", logger.StackTrace(zap.String("user_id", userID.String()), zap.String("session_id", sessionID), zap.Int64("token_version", tokenVersion), zap.Error(err))...)
		return err
	}
	return nil
}
...
func (m *lifecycle) ConsumePasswordChangeClaims(ctx context.Context, claims *authtokens.Claims) error {
	...
	if err := m.passwordChangeSessions.ConsumePasswordChangeSession(ctx, expected); err != nil {
		if errors.Is(err, authdomain.ErrPasswordChangeSessionNotFound) || errors.Is(err, authdomain.ErrPasswordChangeSessionMismatch) {
			logger.Warn(ctx, "password change session rejected", zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID), zap.Error(err))
			return errors.Join(err, authdomain.ErrTokenInvalid)
		}
		logger.Error(ctx, "consume password change session failed", logger.StackTrace(zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID), zap.Error(err))...)
		return err
	}
	return nil
}
...
func (m *lifecycle) ValidateRefreshSession(ctx context.Context, claims *authtokens.Claims) (authdomain.AuthSession, int64, error) {
	...
	if err != nil {
		if errors.Is(err, authdomain.ErrAuthSessionNotFound) {
			logger.Warn(ctx, "refresh session not found", zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID))
			return authdomain.AuthSession{}, 0, errors.Join(authdomain.ErrAuthSessionNotFound, authdomain.ErrTokenInvalid)
		}
		logger.Error(ctx, "get refresh session failed", logger.StackTrace(zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID), zap.Error(err))...)
	...
	if err := authvalidators.ValidateRefreshSessionClaims(session, claims.UserID, claims.TokenVersion); err != nil {
		logger.Warn(ctx, "refresh session mismatch", zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID), zap.Int64("session_token_version", session.TokenVersion), zap.Int64("token_version", claims.TokenVersion))
		return authdomain.AuthSession{}, 0, err
	}
	currentVersion, err := m.CurrentTokenVersion(ctx, claims.UserID)
	if err != nil {
		if errors.Is(err, identity.ErrUserNotFound) {
			logger.Warn(ctx, "refresh user not found", zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID))
			return authdomain.AuthSession{}, 0, identity.ErrUserNotFound
		}
		logger.Error(ctx, "get refresh token version failed", logger.StackTrace(zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID), zap.Error(err))...)
		return authdomain.AuthSession{}, 0, err
	}
	if err := authvalidators.ValidateTokenVersionMatch(currentVersion, session.TokenVersion); err != nil {
		logger.Warn(ctx, "refresh token version mismatch", zap.String("user_id", claims.UserID.String()), zap.String("session_id", claims.SessionID), zap.Int64("current_token_version", currentVersion), zap.Int64("session_token_version", session.TokenVersion))
		return authdomain.AuthSession{}, 0, err
	}
...
func (m *lifecycle) RotateTokenSession(ctx context.Context, oldSession authdomain.AuthSession, newSession authdomain.AuthSession, refreshTTL time.Duration) error {
	if err := m.sessions.RotateSession(ctx, oldSession, newSession, refreshTTL, m.maxActiveSessionsPerUser); err != nil {
		if errors.Is(err, authdomain.ErrAuthSessionNotFound) || errors.Is(err, authdomain.ErrAuthSessionMismatch) {
			logger.Warn(ctx, "rotate auth session rejected", zap.String("user_id", oldSession.UserID.String()), zap.String("old_session_id", oldSession.SessionID), zap.String("new_session_id", newSession.SessionID), zap.Error(err))
			return errors.Join(err, authdomain.ErrTokenInvalid)
		}
		logger.Error(ctx, "rotate auth session failed", logger.StackTrace(zap.String("user_id", oldSession.UserID.String()), zap.String("old_session_id", oldSession.SessionID), zap.String("new_session_id", newSession.SessionID), zap.Error(err))...)
		return err
	}
	return nil
}
...
func (m *lifecycle) DeleteSession(ctx context.Context, userID uuid.UUID, sessionID string) error {
	if err := m.sessions.DeleteSession(ctx, userID, sessionID); err != nil {
		logger.Error(ctx, "delete auth session failed", logger.StackTrace(zap.String("user_id", userID.String()), zap.String("session_id", sessionID), zap.Error(err))...)
		return err
	}
	return nil
}
