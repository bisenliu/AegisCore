...
`user-service/internal/persistence/ent/` 包含用户服务的 Ent schema 源文件、Ent 代码生成入口，以及 repository 层使用的 Ent 生成代码。该目录位于 `internal` 下，属于 user-service 私有持久化实现，不作为跨模块公共 API 暴露。
...
除 `schema/`、`generate.go` 和本文档外，`user-service/internal/persistence/ent/` 下其他文件和目录均属于 Ent 生成代码。这包括顶层生成 Go 文件，例如 `client.go`、`ent.go`、`mutation.go`、`tx.go`、各 Entity 的操作文件，以及 `enttest/`、`hook/`、`migrate/`、`predicate/`、`runtime/` 和 Entity 查询包等生成目录。
...
```bash
go generate ./internal/persistence/ent
```
...
1. 按现有包组织在 `user-service/internal/persistence/ent/schema/` 下新增 schema 源文件。
2. 通过 `user-service/internal/persistence/ent/schema` 包暴露新的 schema，使 Ent codegen 和 Atlas 能从稳定 schema source 读取。
