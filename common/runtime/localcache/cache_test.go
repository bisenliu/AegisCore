...
func TestLoadingCacheUsesFixedTTLWithoutTouchOnHit(t *testing.T) {
	...
	var value int
	started := time.Now()
	require.Eventually(t, func() bool { return time.Since(started) >= 60*time.Millisecond }, time.Second, time.Millisecond)
	value, err = cache.Get(context.Background(), "key")
	require.NoError(t, err)
	require.Equal(t, 1, value)
	require.Eventually(t, func() bool { return time.Since(started) >= 120*time.Millisecond }, time.Second, time.Millisecond)
	value, err = cache.Get(context.Background(), "key")
	require.NoError(t, err)
...
func TestLoadingCacheExpiryAndInvalidationAreNotCapacityEvictions(t *testing.T) {
	...
	require.NoError(t, err)
	started := time.Now()
	require.Eventually(t, func() bool { return time.Since(started) >= 30*time.Millisecond }, time.Second, time.Millisecond)
	_, err = cache.Get(context.Background(), "key")
	require.NoError(t, err)
	cache.Invalidate("key")
	_, err = cache.Get(context.Background(), "key")
