...
- Log 日志消息内容必须全部使用英文，日志字段名使用稳定英文 snake_case；HTTP trace 传播使用 W3C `traceparent` / `tracestate`，日志字段 `trace_id` 来源于当前 OTel span context；HTTP access log 标准字段为 `trace_id`、`user_id`、`client_ip`、`method`、`path`、`status`、`latency_ms`，认证失败安全事件日志额外记录 `user_agent`，但不得记录 password、token、Authorization header、Cookie 或原始请求体；日志级别必须匹配场景严重性，预期业务拒绝不得用 `Error`，系统异常、外部依赖失败、后台任务失败和 panic recover 不得降级为 `Info`。业务日志优先使用 `common/runtime/logger` context helper，避免丢失 trace context。
