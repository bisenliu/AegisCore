...
- Log 日志消息内容必须全部使用英文，日志字段名使用稳定英文 snake_case；日志级别必须匹配场景严重性，预期业务拒绝不得用 `Error`，系统异常、外部依赖失败、后台任务失败和 panic recover 不得降级为 `Info`。业务日志优先使用 `common/runtime/logger` context helper，避免丢失 trace-id。
