...
- `user-services/`：用户服务 HTTP 运行时，包含 Cobra 入口、Fx 组装、Gin 路由、用户 controller/service/repository、Ent schema 和生成代码。
...
- `shared-infrastructure`：加载配置并提供日志、Redis、Postgres、Ent client 等基础设施。
...
- 配置通过 YAML 与 `AEGISCORE_` 环境变量覆盖加载，避免硬编码运行时配置。
