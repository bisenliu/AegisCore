...
import (
	...
	authpkg "github.com/aegiscore/user-services/internal/auth"
...
func TestSessionStoreTokenVersionCacheMiss(t *testing.T) {
	...
	if !errors.Is(err, authpkg.ErrTokenVersionCacheMiss) {
...
func TestTokenVersionValidatorBackfillsMiniredisCacheOnMiss(t *testing.T) {
	...
	validator := authpkg.NewTokenVersionValidator(users, store)
	ctx := context.Background()
...
func TestTokenVersionValidatorRejectsStaleTokenUsingMiniredisCache(t *testing.T) {
	...
	if !errors.Is(err, authpkg.ErrTokenVersionMismatch) {
...
func TestTokenVersionCacheRefreshMakesStaleTokenObservable(t *testing.T) {
	...
	if err := store.CreateSession(ctx, authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-stale", TokenVersion: 5}, time.Hour); err != nil {
	...
	validator := authpkg.NewTokenVersionValidator(&tokenVersionRepositoryStub{err: errors.New("database should not be read")}, store)
...
func TestSessionStoreCreateGetAndDeleteSession(t *testing.T) {
	...
	session := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-123", TokenVersion: 1, ExpiresAt: mismatchedExpiresAt}
...
func TestSessionStoreRotateSession(t *testing.T) {
	...
	indexKey := store.userSessionsKey(sessionTestUserID.String())
	ttl := time.Hour
	oldSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-old", TokenVersion: 1}
	newSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-new", TokenVersion: 1, ExpiresAt: time.Now().Add(24 * time.Hour)}
...
func TestSessionStoreRotateSessionRejectsMissingOldSession(t *testing.T) {
	...
	store := newTestSessionStore(redisServer)
	ctx := context.Background()
	oldSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-missing", TokenVersion: 1}
	newSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-new", TokenVersion: 1}

	err := store.RotateSession(ctx, oldSession, newSession, time.Hour)

	if !errors.Is(err, authpkg.ErrAuthSessionNotFound) {
...
func TestSessionStoreRotateSessionRejectsOldSessionMismatch(t *testing.T) {
	...
	store := newTestSessionStore(redisServer)
	ctx := context.Background()
	storedOldSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-old", TokenVersion: 1}
	if err := store.CreateSession(ctx, storedOldSession, time.Hour); err != nil {
		t.Fatalf("CreateSession: %v", err)
	}
	oldSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-old", TokenVersion: 2}
	newSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-new", TokenVersion: 2}

	err := store.RotateSession(ctx, oldSession, newSession, time.Hour)

	if !errors.Is(err, authpkg.ErrAuthSessionMismatch) {
...
func TestSessionStoreRotateSessionConcurrentAttemptsSucceedOnce(t *testing.T) {
	...
	for i := 0; i < attempts; i++ {
		...
		go func() {
			defer wg.Done()
			<-start
			newSession := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-new-" + strconv.Itoa(i), TokenVersion: 1}
...
func TestSessionStoreDeleteAllUserSessions(t *testing.T) {
	...
	for _, sessionID := range []string{"s-1", "s-2"} {
		if err := store.CreateSession(ctx, authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: sessionID, TokenVersion: 1, ExpiresAt: time.Now().Add(time.Hour)}, time.Hour); err != nil {
			t.Fatalf("CreateSession: %v", err)
		}
	}
...
func TestSessionStoreUserSessionsIndexTTLIsNotShortened(t *testing.T) {
	...
	if err := store.CreateSession(ctx, authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "long", TokenVersion: 1}, longTTL); err != nil {
	...
	if err := store.CreateSession(ctx, authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "short", TokenVersion: 1}, shortTTL); err != nil {
...
func TestSessionStoreKeysUseAppNamePrefix(t *testing.T) {
	...
	ctx := context.Background()
	session := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-prefixed", TokenVersion: 7, ExpiresAt: time.Now().Add(time.Hour)}
...
func TestSessionStoreKeysRemainUnprefixedWhenAppNameEmpty(t *testing.T) {
	...
	ctx := context.Background()
	session := authpkg.AuthSession{UserID: sessionTestUserID.String(), SessionID: "s-empty-prefix", TokenVersion: 7, ExpiresAt: time.Now().Add(time.Hour)}
...
func newTestSessionStoreWithConfig(redisServer *miniredis.Miniredis, authCfg config.AuthConfig) *sessionStore {
	...
	return &sessionStore{redis: client, keys: authpkg.NewRedisKeyBuilder(&config.Config{}), tokenVersionCacheTTL: authCfg.TokenVersionCacheTTL}
}
...
func newTestSessionStoreWithAppName(redisServer *miniredis.Miniredis, appName string) *sessionStore {
	...
	store := NewSessionStore(SessionStoreParams{
		Redis: client,
		Keys:  authpkg.NewRedisKeyBuilder(&config.Config{App: config.AppConfig{Name: appName}}),
		Cfg: &config.Config{
