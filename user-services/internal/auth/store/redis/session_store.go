...
func (r *sessionStore) GetCachedTokenVersion(ctx context.Context, userID string) (int64, error) {
	...
	if errors.Is(err, rediscache.Nil) {
		return 0, authpkg.ErrTokenVersionCacheMiss
	}
	if err != nil {
...
func (r *sessionStore) CreateSession(ctx context.Context, session authpkg.AuthSession, ttl time.Duration) error {
	if ttl <= 0 {
...
func (r *sessionStore) RotateSession(ctx context.Context, oldSession authpkg.AuthSession, newSession authpkg.AuthSession, ttl time.Duration) error {
	if newSession.UserID != oldSession.UserID || newSession.TokenVersion != oldSession.TokenVersion {
		return authpkg.ErrAuthSessionMismatch
	}
	if ttl <= 0 {
		// 非正数 TTL 回退到短期会话，避免创建永久 Redis key。
		ttl = defaultAuthSessionTTL
	...
	if errors.Is(err, rediscache.TxFailedErr) {
		return authpkg.ErrAuthSessionNotFound
	}
	if err != nil {
		return err
	}
...
func (r *sessionStore) GetSession(ctx context.Context, sessionID string) (authpkg.AuthSession, error) {
	data, err := r.redis.Get(ctx, r.sessionKey(sessionID)).Bytes()
	if errors.Is(err, rediscache.Nil) {
		return authpkg.AuthSession{}, authpkg.ErrAuthSessionNotFound
	}
	if err != nil {
		return authpkg.AuthSession{}, fmt.Errorf("get auth session: %w", err)
	}
	var session authpkg.AuthSession
	if err := json.Unmarshal(data, &session); err != nil {
		return authpkg.AuthSession{}, fmt.Errorf("unmarshal auth session: %w", err)
	}
	return session, nil
}
...
func getWatchedSession(ctx context.Context, tx *rediscache.Tx, key string) (authpkg.AuthSession, error) {
