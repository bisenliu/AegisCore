...
func (v *credentialVerifier) VerifyPassword(ctx context.Context, username string, plainPassword string) (*UserCredential, error) {
...
func (v *credentialVerifier) ChangePassword(ctx context.Context, userID uuid.UUID, newPassword string) (*CredentialUpdateResult, error) {
	...
	tokenVersion, err := v.repo.UpdateCredentials(ctx, UpdateCredentialsInput{UserID: userID, PasswordHash: passwordHash, Status: user.UserStatusNormal})
	if err != nil {
		if errors.Is(err, user.ErrUserNotFound) {
