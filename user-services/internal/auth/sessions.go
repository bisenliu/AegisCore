...
func (m *authSessionLifecycle) CreateTokenSession(ctx context.Context, userID string, sessionID string, tokenVersion int64, refreshTTL time.Duration) error {
	if err := m.sessions.CreateSession(ctx, AuthSession{UserID: userID, SessionID: sessionID, TokenVersion: tokenVersion, ExpiresAt: time.Now().Add(refreshTTL)}, refreshTTL); err != nil {
...
func (m *authSessionLifecycle) ValidateRefreshSession(ctx context.Context, claims *commonauth.Claims) (AuthSession, int64, error) {
	...
	if err != nil {
		if errors.Is(err, ErrAuthSessionNotFound) {
			...
			return AuthSession{}, 0, response.TokenInvalidError(messages.MissingSession)
		}
		...
		return AuthSession{}, 0, response.FromError(err)
	}
	...
	if err != nil {
		if errors.Is(err, user.ErrUserNotFound) {
			...
			return AuthSession{}, 0, response.NotFoundError(messages.UserNotFound)
		}
...
func (m *authSessionLifecycle) RotateTokenSession(ctx context.Context, oldSession AuthSession, newSession AuthSession, refreshTTL time.Duration) error {
	if err := m.sessions.RotateSession(ctx, oldSession, newSession, refreshTTL); err != nil {
		if errors.Is(err, ErrAuthSessionNotFound) || errors.Is(err, ErrAuthSessionMismatch) {
...
func currentTokenVersion(ctx context.Context, users UserTokenVersionStore, sessions AuthSessionStore, userID string) (int64, error) {
	...
	if !errors.Is(err, ErrTokenVersionCacheMiss) {
		return 0, err
	}
