...
func TestCredentialVerifierAcceptsMustChangePasswordUser(t *testing.T) {
	...
	verifier := newCredentialVerifier(&authRepoStub{userByUsername: &UserCredential{UserID: authTestUserID, Username: "alice", PasswordHash: passwordHash, Status: user.UserStatusMustChangePassword, TokenVersion: 2}})
...
func TestCredentialVerifierRejectsDisabledUser(t *testing.T) {
	...
	verifier := newCredentialVerifier(&authRepoStub{userByUsername: &UserCredential{UserID: authTestUserID, Username: "alice", PasswordHash: passwordHash, Status: user.UserStatusDisabled, TokenVersion: 2}})
...
func TestCredentialVerifierChangePasswordUpdatesCredentials(t *testing.T) {
	...
	repo := &authRepoStub{userByID: &UserCredential{UserID: authTestUserID, PasswordHash: oldHash, Status: user.UserStatusMustChangePassword, TokenVersion: 2}, newVersion: 3}
	...
	if repo.updatedInput.UserID != authTestUserID || repo.updatedInput.Status != user.UserStatusNormal {
...
func TestCredentialVerifierChangePasswordRejectsInvalidStatus(t *testing.T) {
	verifier := newCredentialVerifier(&authRepoStub{userByID: &UserCredential{UserID: authTestUserID, Status: user.UserStatusNormal, TokenVersion: 2}})
...
func TestCredentialVerifierChangePasswordMapsUpdateError(t *testing.T) {
	verifier := newCredentialVerifier(&authRepoStub{userByID: &UserCredential{UserID: authTestUserID, Status: user.UserStatusMustChangePassword, TokenVersion: 2}, updateErr: errors.New("update failed")})
...
func TestAuthSessionLifecycleRotateTokenSessionMapsRejectedSession(t *testing.T) {
	for _, err := range []error{ErrAuthSessionNotFound, ErrAuthSessionMismatch} {
...
func TestAuthSessionLifecycleRevokeAllUserSessionsMapsUserNotFound(t *testing.T) {
	store := &sessionStoreStub{}
	lifecycle := newAuthSessionLifecycle(&authRepoStub{incrementErr: user.ErrUserNotFound}, store)
...
func authRefreshTestSession(sessionID string, tokenVersion int64) AuthSession {
	return AuthSession{UserID: authTestUserID.String(), SessionID: sessionID, TokenVersion: tokenVersion}
}
