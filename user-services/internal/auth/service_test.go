...
func TestAuthServiceLogin(t *testing.T) {
	...
	repo := &authRepoStub{userByUsername: &UserCredential{UserID: authTestUserID, Username: "alice", PasswordHash: passwordHash, Status: user.UserStatusNormal, TokenVersion: 2}}
...
func TestAuthServiceLoginRejectsInvalidCredentials(t *testing.T) {
	...
	svc := newTestAuthService(&authRepoStub{userByUsername: &UserCredential{UserID: authTestUserID, Username: "alice", PasswordHash: passwordHash, Status: user.UserStatusNormal, TokenVersion: 2}}, &sessionStoreStub{version: 2}, true)
...
func TestAuthServiceLoginRejectsInactiveStatuses(t *testing.T) {
	...
	for _, status := range []user.UserStatus{user.UserStatusDisabled} {
		svc := newTestAuthService(&authRepoStub{userByUsername: &UserCredential{UserID: authTestUserID, Username: "alice", PasswordHash: passwordHash, Status: status, TokenVersion: 2}}, &sessionStoreStub{version: 2}, true)
...
func TestAuthServiceLoginIssuesPasswordChangeToken(t *testing.T) {
	...
	repo := &authRepoStub{userByUsername: &UserCredential{UserID: authTestUserID, Username: "alice", PasswordHash: passwordHash, Status: user.UserStatusMustChangePassword, TokenVersion: 2}}
	store := &sessionStoreStub{version: 2}
	svc := newTestAuthService(repo, store, true)
...
func TestAuthServiceChangePassword(t *testing.T) {
	...
	repo := &authRepoStub{userByID: &UserCredential{UserID: authTestUserID, Username: "alice", PasswordHash: passwordHash, Status: user.UserStatusMustChangePassword, TokenVersion: 2}, newVersion: 3}
	store := &sessionStoreStub{version: 2}
	svc := newTestAuthService(repo, store, true)
	...
	if !result.Changed || repo.updatedInput.UserID != authTestUserID || repo.updatedInput.Status != user.UserStatusNormal || repo.incrementedUserID != authTestUserID || !store.cached || store.cachedVersion != 3 || !store.deletedAll {
...
func TestAuthServiceChangePasswordMapsCredentialUpdateNotFound(t *testing.T) {
	repo := &authRepoStub{userByID: &UserCredential{UserID: authTestUserID, Status: user.UserStatusMustChangePassword, TokenVersion: 2}, updateErr: user.ErrUserNotFound}
	store := &sessionStoreStub{version: 2}
	svc := newTestAuthService(repo, store, true)
	token, err := testJWTService().SignPasswordChangeToken(commonauth.SignInput{UserID: authTestUserID.String(), TokenVersion: 2, SessionID: "pc-123", TTL: time.Hour})
	if err != nil {
...
func TestAuthServiceChangePasswordMapsTokenVersionUserNotFound(t *testing.T) {
	svc := newTestAuthService(&authRepoStub{tokenVersionErr: user.ErrUserNotFound}, &sessionStoreStub{cacheMiss: true}, true)
	token, err := testJWTService().SignPasswordChangeToken(commonauth.SignInput{UserID: authTestUserID.String(), TokenVersion: 2, SessionID: "pc-123", TTL: time.Hour})
	if err != nil {
		t.Fatalf("SignPasswordChangeToken: %v", err)
	}
...
func TestAuthServiceChangePasswordRejectsAccessToken(t *testing.T) {
	repo := &authRepoStub{userByID: &UserCredential{UserID: authTestUserID, Status: user.UserStatusMustChangePassword, TokenVersion: 2}}
	store := &sessionStoreStub{version: 2}
	svc := newTestAuthService(repo, store, true)
...
func TestAuthServiceRefreshRotatesSession(t *testing.T) {
	store := &sessionStoreStub{version: 2, session: AuthSession{UserID: authTestUserID.String(), SessionID: "s-old", TokenVersion: 2}}
...
func TestAuthServiceRefreshRejectsVersionChange(t *testing.T) {
	store := &sessionStoreStub{version: 3, session: AuthSession{UserID: authTestUserID.String(), SessionID: "s-old", TokenVersion: 2}}
	svc := newTestAuthService(&authRepoStub{}, store, true)
	refresh, err := testJWTService().SignRefreshToken(commonauth.SignInput{UserID: authTestUserID.String(), TokenVersion: 2, SessionID: "s-old", TTL: time.Hour})
	if err != nil {
		t.Fatalf("SignRefreshToken: %v", err)
...
func TestAuthServiceRefreshMapsTokenVersionUserNotFound(t *testing.T) {
	store := &sessionStoreStub{session: AuthSession{UserID: authTestUserID.String(), SessionID: "s-old", TokenVersion: 2}, cacheMiss: true}
	svc := newTestAuthService(&authRepoStub{tokenVersionErr: user.ErrUserNotFound}, store, true)
	refresh, err := testJWTService().SignRefreshToken(commonauth.SignInput{UserID: authTestUserID.String(), TokenVersion: 2, SessionID: "s-old", TTL: time.Hour})
	if err != nil {
		t.Fatalf("SignRefreshToken: %v", err)
	}
...
func TestAuthServiceLogoutAllMapsIncrementUserNotFound(t *testing.T) {
	repo := &authRepoStub{incrementErr: user.ErrUserNotFound}
	store := &sessionStoreStub{version: 2}
	svc := newTestAuthService(repo, store, true)
	ctx := commonauth.WithSessionID(commonauth.WithUserID(context.Background(), authTestUserID.String()), "s-123")
...
type authRepoStub struct {
	userByUsername    *UserCredential
	userByID          *UserCredential
	gotUsername       string
	newVersion        int64
	incrementErr      error
	updateErr         error
	incrementedUserID uuid.UUID
	updatedInput      UpdateCredentialsInput
	tokenVersion      int64
	tokenVersionErr   error
	getTokenVersionID uuid.UUID
}

func (r *authRepoStub) GetCredentialByUserID(_ context.Context, userID uuid.UUID) (*UserCredential, error) {
	if r.userByID == nil {
		return nil, user.ErrUserNotFound
	}
	return r.userByID, nil
}
func (r *authRepoStub) GetByUsername(_ context.Context, username string) (*UserCredential, error) {
...
func (r *authRepoStub) UpdateCredentials(_ context.Context, input UpdateCredentialsInput) (int64, error) {
...
func (s *sessionStoreStub) GetCachedTokenVersion(context.Context, string) (int64, error) {
	...
	if s.cacheMiss {
		return 0, ErrTokenVersionCacheMiss
	}
...
func (s *sessionStoreStub) CreateSession(_ context.Context, session AuthSession, ttl time.Duration) error {
	s.created = session
	s.createdTTL = ttl
	return nil
}
func (s *sessionStoreStub) RotateSession(_ context.Context, oldSession AuthSession, newSession AuthSession, ttl time.Duration) error {
	if s.rotateErr != nil {
		return s.rotateErr
	}
	s.deleted = true
...
func (s *sessionStoreStub) GetSession(context.Context, string) (AuthSession, error) {
...
func (m *refreshRotationSessionLifecycle) ValidateRefreshSession(context.Context, *commonauth.Claims) (AuthSession, int64, error) {
	return AuthSession{UserID: authTestUserID.String(), SessionID: "s-old", TokenVersion: 2}, 2, nil
}

func (m *refreshRotationSessionLifecycle) RotateTokenSession(_ context.Context, oldSession AuthSession, newSession AuthSession, _ time.Duration) error {
	if m.createErr != nil {
		return m.createErr
	}
	if m.rotateErr != nil {
