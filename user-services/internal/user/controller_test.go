...
func TestUserControllerGetByUserID(t *testing.T) {
	...
	t.Run("valid ID", func(t *testing.T) {
		...
		service := &stubUserService{response: &UserResponse{UserID: controllerTestUserID, Nickname: "Aegis", Username: "aegis", Status: UserStatusNormal, CreatedAt: createdAt, UpdatedAt: updatedAt}}
		...
		if data["user_id"] != controllerTestUserID || data["nickname"] != "Aegis" || data["username"] != "aegis" || data["status"] != float64(UserStatusNormal) || data["created_at"] != float64(createdAt) || data["updated_at"] != float64(updatedAt) {
...
func TestUserControllerCreate(t *testing.T) {
	...
	createdAt := int64(1780048800000)
	createdUser := &UserResponse{UserID: controllerTestUserID, Nickname: "Alice", Username: "alice", Status: UserStatusNormal, CreatedAt: createdAt, UpdatedAt: createdAt}
	...
	t.Run("valid body", func(t *testing.T) {
		...
		if service.gotCreate.Nickname != "Alice" || service.gotCreate.Username != "alice" || service.gotCreate.Password != "secret" || service.gotCreate.Status == nil || *service.gotCreate.Status != UserStatusNormal {
		...
		data, ok := envelope.Data.(map[string]any)
		if !ok || data["user_id"] != controllerTestUserID || data["nickname"] != "Alice" || data["username"] != "alice" || data["status"] != float64(UserStatusNormal) || data["created_at"] != float64(createdAt) {
...
func TestUserControllerList(t *testing.T) {
	...
	createdAt := int64(1780048800000)
	listResponse := response.NewPaginatedData([]UserResponse{{UserID: controllerTestUserID, Nickname: "Alice", Username: "alice", Status: UserStatusNormal, CreatedAt: createdAt, UpdatedAt: createdAt}}, response.NewPagination(1, 20, 128))

	t.Run("default pagination", func(t *testing.T) {
		service := &stubUserService{listResponse: response.NewPaginatedData([]UserResponse{}, response.NewPagination(1, 10, 0))}
	...
	t.Run("explicit query", func(t *testing.T) {
		...
		if service.gotList.Status == nil || *service.gotList.Status != UserStatusNormal {
...
type stubUserService struct {
	...
	listResponse   response.PaginatedData[UserResponse]
	listErr        error
	gotList        ListUsersQuery
}

func (s *stubUserService) CreateUser(_ context.Context, req CreateUserCommand) (*UserResponse, error) {
	s.gotCreate = req
	if s.createErr != nil {
...
func (s *stubUserService) GetUserByID(_ context.Context, userID uuid.UUID) (*UserResponse, error) {
	s.gotID = userID
	if s.err != nil {
...
func (s *stubUserService) ListUsers(_ context.Context, req ListUsersQuery) (response.PaginatedData[UserResponse], error) {
	s.gotList = req
	if s.listErr != nil {
		return response.PaginatedData[UserResponse]{}, response.FromError(s.listErr)
	}
...
func assertPaginatedEnvelope(t *testing.T, envelope response.Envelope, page, pageSize, total, totalPages, itemCount int) {
	...
	if itemCount > 0 {
		...
		if !ok || item["user_id"] != controllerTestUserID || item["nickname"] != "Alice" || item["username"] != "alice" || item["status"] != float64(UserStatusNormal) {
