...
- Redis publish 或 Pub/Sub 故障后恢复：验证数据库 policy revision 已提交但通知链路失败时，dispatcher 重试和 watcher 数据库 policy revision 补偿可在没有新 RBAC 写入的情况下使两个副本 lag 归零，并通过恢复前 deny、恢复后 allow 证明 Casbin projection 已更新；纯 user-role revision 只验证用户角色缓存失效和回源收敛，不要求 Casbin policy reload。
...
- Add/Remove/Replace 重放：验证 dispatcher 重试、重复投递和乱序 watcher 事件不会丢通知，也不会因非幂等副作用破坏最终 projection 或 cache 失效语义；重复 policy 通知必须被合并到最高未应用 policy revision，重复 user-role 通知不得查询 `role_permissions` 全集。
- 100 并发 RBAC 写：使用真实 PostgreSQL mutation 验证业务行、commit-ordered policy/user-role revision 和 pending outbox 一一对应且各自 revision 连续唯一；授权收敛由独立的 Redis 故障恢复 e2e 断言覆盖。
