...
| `common-credentials` | 统一管理 common 模块内密码、JWT token、认证传输常量和认证上下文绑定等共享凭证原语 | `common/credentials/` | `openspec/specs/common-credentials/spec.md` | proposed |
| `user-authentication` | 通过 JWT Bearer 中间件校验受保护 API 并传播外部 UUID `user_id` 认证身份 | `common/credentials/`, `common/middleware/auth.go`, `user-services/internal/bootstrap/bootstrap.go` | `openspec/specs/user-authentication/spec.md` | ready |
