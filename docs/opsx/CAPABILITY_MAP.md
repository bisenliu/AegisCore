...
|---|---|---|---|---|
| `user-profile-query` | 通过外部 UUID `user_id` 查询用户资料并返回统一 API 响应 | `user-services/internal/router/router.go`, `user-services/internal/user/controller.go`, `user-services/internal/user/service.go`, `user-services/internal/user/store/postgres/user_store.go`, `user-services/ent/schema/user.go` | `openspec/specs/user-profile-query/spec.md` | ready |
| `user-profile-create` | 使用唯一 `username` 创建用户资料并将请求校验、唯一性冲突和持久化错误转换为统一响应 | `user-services/internal/router/router.go`, `user-services/internal/user/controller.go`, `user-services/internal/user/service.go`, `user-services/internal/user/store/postgres/user_store.go`, `user-services/ent/schema/user.go` | `openspec/specs/user-profile-create/spec.md` | ready |
...
| `user-session-control` | 通过 username 登录、短期 Access Token、可撤销 Refresh Token 会话和用户级 token_version 支持登录、刷新、退出当前设备和退出全部设备 | `user-services/internal/auth/controller.go`, `user-services/internal/auth/service.go`, `user-services/internal/auth/sessions.go`, `user-services/internal/auth/store/redis/session_store.go`, `user-services/internal/user/store/postgres/user_store.go`, `user-services/ent/schema/user.go` | `openspec/specs/user-session-control/spec.md` | proposed |
...
| `api-response-contract` | 统一 HTTP 成功/失败信封、错误码和应用错误映射 | `common/contract/response/`, `common/http/middleware/recovery.go`, `user-services/internal/user/controller.go`, `user-services/internal/auth/controller.go` | `openspec/specs/api-response-contract/spec.md` | ready |
| `request-validation` | 为 Gin controller 提供统一请求绑定、结构体校验、字段明细和校验失败响应 | `common/validation/`, `common/http/ginvalidation/`, `user-services/internal/validators/`, `user-services/internal/user/controller.go`, `user-services/internal/auth/controller.go`, `user-services/internal/bootstrap/app.go` | `openspec/specs/request-validation/spec.md` | ready |
...
| `api-swagger-documentation` | 生成并按环境暴露与运行时路由和响应契约一致的 Swagger/OpenAPI 文档 | `user-services/cmd/main.go`, `user-services/internal/router/swagger.go`, `user-services/docs/`, `user-services/internal/user/controller.go`, `user-services/internal/auth/controller.go` | `openspec/specs/api-swagger-documentation/spec.md` | ready |
...
- User capability：`user-services/internal/user/`
- Auth capability：`user-services/internal/auth/`
